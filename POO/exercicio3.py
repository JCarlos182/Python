...
print("E ae viajante. Temos algumas ofertas para você!")
traveler = input("Digites seu nome para começarmos: ")
...
for option in list_trip:
    if choice >= 5: #caso usuário não digite a opção correta
        print("Esta opção não está incluida em nossos destinos.")
        break
    if choice <= 4: #verifica a seleção
        print(f"{traveler} sua viagem para {list_trip[choice].destiny} está marcada.") # retultado
        break
